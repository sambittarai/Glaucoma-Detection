...
def load_data(data_path_list_file):
    ...
    for i in range(len(img_list)):
        ...
        gt = np.asarray(Image.open(gt_list[i]))
        if len(gt.shape)==3:
            gt = gt[:,:,0]
...
#==============================Load train data==============================================
def get_data_train(data_path_list, patch_height, patch_width, stride_height, stride_width):
    '''
    Load the original data and return the extracted patches for testing, return the ground truth in its original shape (all datasets except drive)
    '''
    ...
    train_imgs = paint_border_overlap(train_imgs, patch_height, patch_width, stride_height, stride_width)
    train_masks = paint_border_overlap(train_masks, patch_height, patch_width, stride_height, stride_width)

    #check masks are within 0-1
    assert(np.max(train_masks)==1  and np.min(train_masks)==0)
    print("\nTrain images shape: {}, value range ({} - {}):"\
        .format(train_imgs.shape, str(np.min(train_imgs)), str(np.max(train_imgs))))
    print("\nTraint masks shape: {}, value range ({} - {}):"\
        .format(train_masks.shape, str(np.min(train_masks)), str(np.max(train_masks))))

    #extract the test patches from the all test images
    patches_imgs_train = extract_ordered_overlap(train_imgs,patch_height,patch_width,stride_height,stride_width)
    patches_masks_train = extract_ordered_overlap(train_masks,patch_height,patch_width,stride_height,stride_width)
    ...
    print("train patches shape: {}, value range ({} - {})"\
        .format(patches_masks_train.shape, str(np.min(patches_masks_train)), str(np.max(patches_masks_train))))
